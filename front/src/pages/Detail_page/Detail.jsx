...
const Detail = ({tickerData}) => {
  ...
  const goToCommunityPage = () => {
    const company = Object.values(companyInfo).find(
      (company) => company.name.toUpperCase() === info.name.toUpperCase()
    );
    if (company) {
      navigate(`/channel/${company.name}`);
    } else {
      // 해당하는 회사 정보가 없을 경우
      console.error(`해당 게시판이 없습니다 ${ticker}`);
    }
  };
    ...
    <div style={styles.mystockContainer}>
      <div style={styles.title}>
        <div style={styles.titleContent}>
          <h2>{info.name}</h2>
          <HeartButton style={styles.heartButton} item={info.name}/>
        </div>
      </div>
      <div style={styles.downsection}>
        <div style={styles.leftsection}>
          <div style={styles.text}>{info.description}</div>
        </div>
